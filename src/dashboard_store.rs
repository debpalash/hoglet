...
impl DashboardStore {
    ...
    pub fn list_insights(&self, token: &str) -> Result<Vec<SavedInsight>, DashboardError> {
        ...
        let rows = stmt
            .query_map([token], row_to_insight)?
    ...
    pub fn get_insight(&self, id: &str) -> Result<SavedInsight, DashboardError> {
        ...
        stmt.query_row([id], row_to_insight)
    ...
    pub fn save_insight(
    ...
    ) -> Result<SavedInsight, DashboardError> {
        let conn = self.conn.lock().unwrap();
        ...
        Ok(SavedInsight {
            ...
            query_ir: insight.query_ir.clone(),
            created_by: insight.created_by.clone(),
            created_at: if insight.id.is_empty() {
                now
            } else {
                insight.created_at
            },
        ...
        dash.tiles = tile_stmt
            .query_map([id], |r| {
                ...
                let insight = insight_name.map(|name| SavedInsight {
                    ...
                    token: dash.token.clone(),
                    name,
                    description: r.get::<_, String>(6).unwrap_or_default(),
                    query_ir: serde_json::from_str(&r.get::<_, String>(7).unwrap_or_default())
    ...
    pub fn save_dashboard(
        ...
        for tile in &dashboard.tiles {
            ...
            tile_stmt.execute(rusqlite::params![
                tid,
                id,
                tile.insight_id,
                tile.x,
        ...
        Ok(Dashboard {
            ...
            created_by: dashboard.created_by.clone(),
    ...
    pub fn create_share(
        ...
        object_id: &str,
        created_by: &str,
    ) -> Result<ShareLink, DashboardError> {
        let conn = self.conn.lock().unwrap();
...
mod tests {
    ...
    fn insight_save_and_list() {
        ...
        let saved = store
            .save_insight("phc_t", &example_insight("phc_t"))
            .unwrap();
    ...
    fn dashboard_with_tiles() {
        ...
        assert_eq!(
            loaded.tiles[0].insight.as_ref().unwrap().name,
            "Test insight"
        );
    }
